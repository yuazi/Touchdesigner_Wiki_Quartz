...
3.  **Teacher Forcing Bias (Exposure Bias)**: If you only ever train the decoder using _correct_ ground truth tokens, it might not know how to recover when it makes a single mistake during inference. It’s like a student who only ever sees the solution key and panics when they have to solve a problem from scratch.
4.  **Sequential Bottleneck**: RNNs _cannot_ be parallelized over time. You must compute $h_t$ before you can compute $h_{t+1}$, which is why they are often slower to train than Transformers.
