---
title: "10_global_illumination  -  Real-Time Global Illumination"
tags:
  - rtg
  - global-illumination
  - radiosity
...
[[notes/lectures/realtimegraphics/09_semi_global_illumination|Back: (y-09) Semi-Global Illumination]] | [[notes/lectures/realtimegraphics/index|RTG Index]] | [[notes/lectures/realtimegraphics/11_gpu_raytracing|Next: (y-11) GPU Raytracing]]
...
![[pictures/realtimegraphics/10/L10_Pg-04.jpg]]

<p class="image-caption">L10_Pg-04: The global effects ordered by complexity  -  hard shadows, reflections, refractions, indirect illumination, ambient occlusion, caustics.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-08.jpg]]

<p class="image-caption">L10_Pg-08: Two axes  -  light-transport complexity (shadows, diffuse GI, specular GI) and scene dynamics (static scene + moving camera, moving objects, fully dynamic). Each combination admits different precomputation strategies.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-06.jpg]]

<p class="image-caption">L10_Pg-06: Indirect illumination fills in the dark side of the Cornell box  -  the yellow rectangles bouncing red and blue tint into the scene.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-07.jpg]]

<p class="image-caption">L10_Pg-07: Indirect shadows are non-local  -  a sheet that occludes the bounced light path between two surfaces casts a shadow that does not match any direct-light shadow.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-10.jpg]]

<p class="image-caption">L10_Pg-10: Kajiya's rendering equation  -  outgoing radiance equals emission plus the integral over the hemisphere of incoming radiance times BRDF.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-11.jpg]]

<p class="image-caption">L10_Pg-11: The BRDF $f_r(\mathbf{x}, \boldsymbol{\omega}_i, \boldsymbol{\omega}_o)$ describes how a surface at $\mathbf{x}$ converts incoming light from $\boldsymbol{\omega}_i$ to outgoing light in $\boldsymbol{\omega}_o$.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-15.jpg]]

<p class="image-caption">L10_Pg-15: For a purely diffuse surface, irradiance $E(\mathbf{x}) = \int L(\mathbf{x}, \boldsymbol{\omega}_i)(\mathbf{n} \cdot \boldsymbol{\omega}_i) \, d\boldsymbol{\omega}_i$. Irradiance is indexed by surface normal; radiance is indexed by incoming direction.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-16.jpg]]

<p class="image-caption">L10_Pg-16: The recursive rendering equation expanded as $L = L_0 + L_1 + \ldots$, with $L_0$ direct lighting and $L_1$ one-bounce indirect. The direct-only image is mostly black; the indirect-only image has all the colour bleeding.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-17.jpg]]

<p class="image-caption">L10_Pg-17: Path tracing follows ray paths between camera and light  -  the reference solution. Monte Carlo sampling means noisy results that converge to ground truth.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-23.jpg]]

<p class="image-caption">L10_Pg-23: A Cornell-box-style scene rendered with radiosity  -  soft shadows and red/green colour bleeding from the walls.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-24.jpg]]

<p class="image-caption">L10_Pg-24: Scene split into $n$ patches $P_k$, each with constant radiosity $B_k$. Discrete radiosity equation: $B_k = E_k + \rho_k \sum_{j \ne k} F_{j,k} B_j$.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-25.jpg]]

<p class="image-caption">L10_Pg-25: Form factors are computed by numeric integration (Monte Carlo ray tracing) or by the hemicube method  -  rasterise the scene onto a half-cube around the receiver patch, using the depth buffer for occlusion.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-28.jpg]]

<p class="image-caption">L10_Pg-28: Gathering vs. shooting  -  gathering collects energy into one patch (one row of the matrix), shooting distributes one patch's energy to all others (one column). Shooting converges faster in practice.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-30.jpg]]

<p class="image-caption">L10_Pg-30: Photon mapping  -  forward rays from the light source land photons on diffuse surfaces, then backward rays from the camera sample them.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-31.jpg]]

<p class="image-caption">L10_Pg-31: At point $x$, count photons within sphere of radius $r$. The kd-tree makes this lookup fast. Caustic patterns emerge naturally because photons cluster where specular paths converge.</p>

![[pictures/realtimegraphics/10/L10_Pg-32.jpg]]

<p class="image-caption">L10_Pg-32: The radius $r$ trades noise for blur  -  $r = 1$ is grainy, $r = 8$ is smooth but loses detail.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-37.jpg]]

<p class="image-caption">L10_Pg-37: Spherical harmonics  -  decompose a function on a sphere into a sum of basis functions (analogue of Fourier series on a 1D interval). Successive approximation by truncating the series.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-39.jpg]]

<p class="image-caption">L10_Pg-39: 16 spherical-harmonic coefficients reconstruct a low-pass version of a cubemap environment. SH gives compact storage (16 floats per colour channel) but only diffuse-quality detail.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-41.jpg]]

<p class="image-caption">L10_Pg-41: Instant radiosity converts indirect lighting into direct lighting from many virtual point lights (VPLs) scattered through the scene.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-42.jpg]]

<p class="image-caption">L10_Pg-42: Pass 1 shoots photons from the light, creates a VPL at each hit, generates a shadow map per VPL. Pass 2 does deferred rendering using all VPLs and their shadow maps.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-45.jpg]]

<p class="image-caption">L10_Pg-45: VPL distribution as photons cascade through the scene  -  weight $\rho^i$ falls off geometrically with bounce $i$.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-46.jpg]]

<p class="image-caption">L10_Pg-46: Sponza atrium lit by instant radiosity  -  warm bounced light fills the arcades.</p>

### Problem: Too Many Shadow Maps

![[pictures/realtimegraphics/10/L10_Pg-49.jpg]]

<p class="image-caption">L10_Pg-49: Imperfect shadow maps  -  VPL shadow maps do not need accurate geometry. Even 20% corrupted depth gives a result close to high-quality.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-50.jpg]]

<p class="image-caption">L10_Pg-50: ISM substitutes triangles with a coarse point cloud per VPL, then fills holes with pull-push interpolation.</p>

![[pictures/realtimegraphics/10/L10_Pg-51.jpg]]

<p class="image-caption">L10_Pg-51: Pull-push interpolation on a sparse point-cloud shadow map turns a noisy depth into a usable one.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-55.jpg]]

<p class="image-caption">L10_Pg-55: ISM at 11 fps next to a hours-long reference path-traced image  -  the difference is small enough to ship.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-58.jpg]]

<p class="image-caption">L10_Pg-58: A reflective shadow map (RSM) is a shadow map that also stores position, normal, and outgoing flux per pixel  -  each pixel becomes a small VPL.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-59.jpg]]

<p class="image-caption">L10_Pg-59: Gather illumination from RSM. Too many RSM pixels to sample all of them, so restrict to samples close to the current pixel's projection into the RSM.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-61.jpg]]

<p class="image-caption">L10_Pg-61: RSM limitations  -  diffuse reflectors only, and the result has no indirect shadows (the cast shadow on the right wall is wrong).</p>
...
![[pictures/realtimegraphics/10/L10_Pg-81.jpg]]

<p class="image-caption">L10_Pg-81: An illumination probe stored as octahedral map of radiance, normals, and distances per direction. Octahedral mapping unfolds a sphere into a square with low distortion.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-82.jpg]]

<p class="image-caption">L10_Pg-82: Trace a ray against the probe by picking the closest probe to the ray and linearly tracing through the octahedron section the ray covers.</p>

![[pictures/realtimegraphics/10/L10_Pg-83.jpg]]

<p class="image-caption">L10_Pg-83: Intersection test  -  if the probe's depth map value is less than the distance from ray origin to probe, the ray has either hit the surface or passed behind it. The normal map disambiguates.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-84.jpg]]

<p class="image-caption">L10_Pg-84: Light leaks happen when the receiver is on the wrong side of a wall relative to the probe. Using per-direction distance $r$ stored in the probe, reject the contribution if the distance from surface to probe exceeds $r$.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-85.jpg]]

<p class="image-caption">L10_Pg-85: A single radiance probe assumes infinitely distant lighting. When the lighting environment is nearby (red and green panels), the single-probe approximation goes flat; interpolating between multiple probes recovers the gradient.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-87.jpg]]

<p class="image-caption">L10_Pg-87: Screen-space probes concentrate samples on visible pixels, hugging geometry precisely so they cannot leak.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-91.jpg]]

<p class="image-caption">L10_Pg-91: Hybrid caching  -  world-space probes store outgoing radiance for shooting (capture infinite bounces over time), screen-space probes store incoming radiance for gathering (compute irradiance at the current pixel).</p>
...
![[pictures/realtimegraphics/10/L10_Pg-92.jpg]]

<p class="image-caption">L10_Pg-92: Cache update strategy by distance (Lumen)  -  ambient occlusion under 16 pixels updates per pixel, screen-space probes (2-16 m) use temporal accumulation, world probes (~200 m) use probe-level caching, skylight is amortised over many frames. Lighting latency grows with distance because the eye is less sensitive there.</p>
...
![[pictures/realtimegraphics/10/L10_Pg-94.jpg]]

<p class="image-caption">L10_Pg-94: PRT factors shading into illumination $L_{in}$ (the environment light) and visibility $V$ (precomputed directional transfer at each surface point). Final shading is the dot product of two SH vectors.</p>
