---
title: "08_special_effects  -  Image-Space Special Effects"
tags:
  - rtg
  - special-effects
  - post-processing
...
[[notes/lectures/realtimegraphics/07_deferred_shading|Back: (y-07) Deferred Shading]] | [[notes/lectures/realtimegraphics/09_semi_global_illumination|Next: (y-09) Semi-Global Illumination]] | [[notes/lectures/realtimegraphics/index|RTG Index]]
...
![[pictures/realtimegraphics/08/L08_Pg-02.jpg]]

<p class="image-caption">L08_Pg-02: Image-space effects extend the deferred pipeline by adding a post-processing stage that reads color and G-Buffer resources.</p>
...
<p class="image-caption">L08_Pg-04: Three fog curves  -  linear, exponential, and squared exponential  -  produce different falloff shapes.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-07.jpg]]

<p class="image-caption">L08_Pg-07: A 5x5 Gaussian kernel costs 25 texture lookups per pixel before normalization by 1/256.</p>
...
<p class="image-caption">L08_Pg-08: A 2D Gaussian is the outer product of two 1D Gaussians, so a 5x5 filter becomes 5+5 = 10 lookups in two passes.</p>
...
<p class="image-caption">L08_Pg-11: Bloom isolates bright pixels with a threshold, blurs them, then adds the blurred glow back on top of the original.</p>
...
<p class="image-caption">L08_Pg-14: DoF simulates a real lens, where objects far from the focal plane progressively blur.</p>

Image-space DoF turns this into a postprocess:

![[pictures/realtimegraphics/08/L08_Pg-17.jpg]]

<p class="image-caption">L08_Pg-17: Per pixel, the circle of confusion comes from depth, then a convolution blurs by that radius  -  but naive filtering leaks foreground into background.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-19.jpg]]

<p class="image-caption">L08_Pg-19: A joint bilateral filter weights samples by spatial distance and by similarity in a guide channel (e.g. depth), giving edge-preserving blur.</p>
...
- A **first-order differential** (Sobel) on the depth buffer picks out depth discontinuities, which correspond to object silhouettes. This is called the _profile_ edge.
- A **second-order differential** (Laplacian) picks out depth inflection points: ridges and creases inside a surface. This is the _internal_ edge.

![[pictures/realtimegraphics/08/L08_Pg-21.jpg]]

<p class="image-caption">L08_Pg-21: Running edge detection on both the depth buffer and the normal buffer catches silhouettes the depth alone misses.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-24.jpg]]

<p class="image-caption">L08_Pg-24: A G-Buffer of normals, screen depth, and surface (u, v) coordinates drives hatching that follows the surface, profile, and curvature.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-27.jpg]]

<p class="image-caption">L08_Pg-27: DLSS is a per-game convolutional auto-encoder that combines temporal anti-aliasing with image upscaling, executed entirely in the GPU driver.</p>
...
<p class="image-caption">L08_Pg-29: SSAA takes many samples per pixel, then accumulates them via a reconstruction filter  -  equivalent to rendering at higher resolution and downsampling.</p>
...
Most visible aliasing is "jaggies" at triangle edges. **MSAA** places multiple subsamples per pixel for _coverage and depth tests_, but runs the fragment shader only once per pixel. The output color is then blended based on how many of the subsamples were covered. Hardware support is built into every graphics API.
...
![[pictures/realtimegraphics/08/L08_Pg-32.jpg]]

<p class="image-caption">L08_Pg-32: MLAA is a three-pass post-process: detect edges, compute a blending texture from line shapes, then blend along edges.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-34.jpg]]

<p class="image-caption">L08_Pg-34: Summary  -  SSAA is universal but expensive, MSAA fixes only edges and breaks with deferred, MLAA is cheap but flickers.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-40.jpg]]

<p class="image-caption">L08_Pg-40: TAA reprojects the previous output (history), validates and rectifies it against current samples, then accumulates the blend, ready to feed the next frame.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-41.jpg]]

<p class="image-caption">L08_Pg-41: Jittering offsets the projection matrix by sub-pixel fractions each frame, so each frame samples a different point inside the pixel.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-43.jpg]]

<p class="image-caption">L08_Pg-43: TAA stores history as an exponential moving average, $s_t = \alpha x_t + (1 - \alpha) s_{t-1}$, giving infinite samples in fixed storage.</p>

A simple moving average of the last $N$ frames would need $N$ times the storage. The trick is an **exponential moving average**:

$$ s*t = \alpha\,x_t + (1 - \alpha)\,s*{t-1} $$
...
![[pictures/realtimegraphics/08/L08_Pg-46.jpg]]

<p class="image-caption">L08_Pg-46: Each vertex is transformed by both the previous and current frame's matrices; their difference is rendered as a per-pixel motion vector texture.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-48.jpg]]

<p class="image-caption">L08_Pg-48: Reprojected history is invalid at occlusion or disocclusion events, or when shading changes  -  use geometry data (depth, normal, ID, motion) or color variance to detect and clear it.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-52.jpg]]

<p class="image-caption">L08_Pg-52: Motion blur is relative to the camera  -  palm trees streak past a moving car while the car itself stays sharp.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-55.jpg]]

<p class="image-caption">L08_Pg-55: Per-pixel velocity = current position - previous position; sample along that direction in the color buffer and accumulate.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-58.jpg]]

<p class="image-caption">L08_Pg-58: Discontinuities at silhouettes and color bleeding from slow foreground into fast background; centring the blur helps.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-62.jpg]]

<p class="image-caption">L08_Pg-62: Lens flare is rendered as alpha-blended billboards strung along the line between the light source and the image centre.</p>
...
![[pictures/realtimegraphics/08/L08_Pg-65.jpg]]

<p class="image-caption">L08_Pg-65: To make a billboard face the camera, replace the rotation portion of the ModelView matrix with identity, keeping translation and scale.</p>
...
The 1982 Genesis sequence in _Star Trek II_ is the canonical historical example.

![[pictures/realtimegraphics/08/L08_Pg-72.jpg]]

<p class="image-caption">L08_Pg-72: A minimal particle struct: lifetime, speed, position, direction, alpha  -  varied over time by a shared update rule.</p>
