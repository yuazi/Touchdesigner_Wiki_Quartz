...
- **Semaphores**: GPU-to-GPU signalling between queues or between submission boundaries. Standard frame setup uses an _image-available_ semaphore (the swap-chain image is acquired and ready to render into) and a _render-finished_ semaphore (rendering is done and the image is safe to present).
