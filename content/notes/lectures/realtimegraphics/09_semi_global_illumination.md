---
title: "09_semi_global_illumination  -  Semi-Global Illumination"
tags:
  - rtg
  - global-illumination
  - shadows
...
[[notes/lectures/realtimegraphics/08_special_effects|Back: (y-08) Image-Space Special Effects]] | [[notes/lectures/realtimegraphics/index|RTG Index]] | [[notes/lectures/realtimegraphics/10_global_illumination|Next: (y-10) Real-Time Global Illumination]]
...
![[pictures/realtimegraphics/09/L09_Pg-006.jpg]]

<p class="image-caption">L09_Pg-006: Stencil-based reflection in four steps  -  write the mirror shape into the stencil, draw the mirrored object clipped to the stencil, then the normal object and the mirror surface on top.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-014.jpg]]

<p class="image-caption">L09_Pg-014: SSR reflects the view ray across the surface normal and finds where that reflection ray intersects the depth buffer  -  a cheap dynamic reflection.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-017.jpg]]

<p class="image-caption">L09_Pg-017: From the initial screen position, march along the reflected screen-space direction until the ray's depth exceeds the depth buffer at that pixel.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-025.jpg]]

<p class="image-caption">L09_Pg-025: Without depth sorting, the left arm appears in front of the body because its triangles happened to be drawn after the body; with sorting, the layering is correct.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-028.jpg]]

<p class="image-caption">L09_Pg-028: Depth peeling renders the scene n times, each pass extracting the next-nearest fragment by comparing depth to the previous layer.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-038.jpg]]

<p class="image-caption">L09_Pg-038: Linked-list creation in progress  -  each new fragment is appended to the head of its pixel's list, with the old head index recorded as the next pointer.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-042.jpg]]

<p class="image-caption">L09_Pg-042: For a heavily transparent dragon scene, linked-list OIT runs ~3.5x faster than depth peeling because it amortizes the geometry pass.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-045.jpg]]

<p class="image-caption">L09_Pg-045: Without shadows, characters and bushes float over the ground; with shadows, they sit on the surface.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-047.jpg]]

<p class="image-caption">L09_Pg-047: A shadowing object carves a shadow volume  -  surfaces inside are shadowed, surfaces outside are illuminated, surfaces partially inside are partially shadowed.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-048.jpg]]

<p class="image-caption">L09_Pg-048: An area light produces an inner umbra (fully occluded) surrounded by a penumbra (partially occluded).</p>
...
![[pictures/realtimegraphics/09/L09_Pg-056.jpg]]

<p class="image-caption">L09_Pg-056: Naive blob shadows either z-fight with the ground (blend at hit polygon), float above it (elevate + z-test less-or-equal), or sit in the wrong place (no z-test).</p>
...
![[pictures/realtimegraphics/09/L09_Pg-058.jpg]]

<p class="image-caption">L09_Pg-058: Projection onto the XZ-plane from a light $\mathbf{l}$ comes out of similar triangles, giving $p_x = (l_y v_x - l_x v_y)/(l_y - v_y)$, $p_y = 0$, and an analogous expression for $p_z$.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-060.jpg]]

<p class="image-caption">L09_Pg-060: Projected shadow artifacts  -  z-fighting with the receiver, double blending where projected triangles overlap, and shadow extending off the ground plane.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-064.jpg]]

<p class="image-caption">L09_Pg-064: For each camera pixel, compute its depth as seen from the light, compare to the shadow map; if the camera-pixel depth is greater, the pixel is in shadow.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-067.jpg]]

<p class="image-caption">L09_Pg-067: Pixels whose camera-light depth matches the shadow map are coloured green (lit); the rest are darker (shadowed).</p>
...
![[pictures/realtimegraphics/09/L09_Pg-068.jpg]]

<p class="image-caption">L09_Pg-068: Shadow-map aliasing has two components  -  perspective aliasing (camera-space texel resolution) and projection aliasing (shadow-map texel resolution under glancing angles).</p>
...
![[pictures/realtimegraphics/09/L09_Pg-076.jpg]]

<p class="image-caption">L09_Pg-076: PCF filters the boolean lit/shadow result of per-texel comparison, not the depth values themselves; the example averages 5 of 9 lit samples to give 0.55 visibility.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-078.jpg]]

<p class="image-caption">L09_Pg-078: Too small a depth offset gives shadow acne (every lit surface flickers in self-shadow); too large gives Peter Panning (shadow detaches from the object).</p>
...
![[pictures/realtimegraphics/09/L09_Pg-079.jpg]]

<p class="image-caption">L09_Pg-079: Slope-scale bias adjusts the depth offset per polygon based on its slope relative to the light, applying a larger offset for steep angles where quantization is worse.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-081.jpg]]

<p class="image-caption">L09_Pg-081: CSM partitions the eye-space frustum into cascading sub-frusta along z, with a separate shadow map per cascade  -  nearby cascades cover little area at high resolution, far cascades cover more at lower resolution.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-084.jpg]]

<p class="image-caption">L09_Pg-084: A dual paraboloid shadow map projects each hemisphere onto a disc  -  two passes instead of six, supported by native texture-coordinate hardware.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-087.jpg]]

<p class="image-caption">L09_Pg-087: For each pixel, count how many shadow-volume polygon fragments pass the depth test along the view ray. An odd count means the pixel is inside the volume (shadowed), even means outside (lit).</p>
...
![[pictures/realtimegraphics/09/L09_Pg-090.jpg]]

<p class="image-caption">L09_Pg-090: For an object inside the shadow volume, the front-face increment outnumbers the back-face decrement by 1  -  the stencil value is non-zero, so the pixel is shadowed.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-093.jpg]]

<p class="image-caption">L09_Pg-093: Shadow volumes need closed-manifold meshes, can produce extreme overdraw, and are fill-rate bound  -  the right image visualizes the wireframe of just the shadow volume polygons.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-099.jpg]]

<p class="image-caption">L09_Pg-099: Ambient occlusion darkens surfaces partially blocked from environmental light  -  independent of light direction, it adds depth and contrast in corners, crevices, and contact areas.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-100.jpg]]

<p class="image-caption">L09_Pg-100: AO weighs the standard shading by $(1 - AO)$  -  a fully unoccluded surface uses normal lighting; a deep crevice with $AO \to 1$ goes dark.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-103.jpg]]

<p class="image-caption">L09_Pg-103: SSAO samples the depth buffer in a hemisphere around each pixel and counts samples whose depth lies "inside" the surface as occluded.</p>
...
![[pictures/realtimegraphics/09/L09_Pg-105.jpg]]

<p class="image-caption">L09_Pg-105: SSDO with first bounce takes the color of each occluder pixel as a small radiating patch, giving cheap color bleeding (red wall casting red onto nearby spheres).</p>
