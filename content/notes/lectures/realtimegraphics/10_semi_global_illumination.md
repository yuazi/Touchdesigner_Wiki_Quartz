...
The modern OIT approach: write **every fragment** produced during rasterization into a global list, then sort and blend later. Historically called the *A-buffer* (accumulation buffer).
...
The standard fix uses a **stencil buffer**: draw the receiver plane writing 1 into stencil, draw the projected shadow only where stencil is 1 (which also writes 0 back), so each shadow pixel is blended exactly once and no shadow extends off the plane. *GLQuake* (1997) was the first game to ship this technique.
...
The alternative shadow technique, due to Crow 1977 and made famous by *Doom 3*, is to compute shadow boundaries **geometrically** instead of sampling them.
