...
**Program Verification** is a formal system used to reason about the correctness of computer programs. Instead of just running the program with some inputs (testing), we use logic to prove it works for _all_ possible inputs.
...
2.  **The Specification**: What the program is _supposed_ to do (e.g., "no division by zero", "array always in bounds").
...
- **Testing is limited**: As the slides show, a simple function like `y / (myHash(x) - 23)` might only crash for _one_ specific value of `x`. You might never find it by random testing.

### 💡 Intuition: Testing vs. Verification

Imagine you are checking if a floor is safe to walk on.

- **Testing**: You jump on a few spots. If they don't break, you _hope_ the rest is fine.
...
The **Halting Problem** tells us we can't write a perfect verifier that works for _every_ possible program.

- **Strategy**: We don't try to solve it for everything. We build tools that are helpful for _most_ programs we care about.
