...
- **Internal Correctness**: Instead of checking a "Pre-Post" pair at the very end, **Assert** statements let us specify correctness _anywhere_ in the code.
...
- **Assert Statements**: You placed "security cameras" (asserts) in every room. If the person is caught doing something wrong in _any_ room, the whole visit is considered a failure.
